...
@synthesize matches, subscription, publicationRadius, me, peerID,subscriptionRadius, likedMessages, sharedTopic, timer, messageLifetime, roomLifetime, speakUpDelegate,dislikedMessages,myMessagePublicationIDs,myRoomIDs,inputText, isSuperUser,matchRepository, messageManagerDelegate, roomManagerDelegate, roomCounter, messageCounter, roomArray, locationIsOK, connectionIsOK,myRoomPublicationIDs, myMessageIDs;
...
+(id) sharedSpeakUpManager{
    @synchronized(self) {
        if (sharedSpeakUpManager == nil){
            ...
            [sharedSpeakUpManager createMobilePeer];
            sharedSpeakUpManager.matchRepository =  [[MatchRepository alloc] initWithRepositoryURI:@"http://isi01.unil.ch:8080/iWall-Middleware-TEST-Bridging/MatchRepository"];
...
-(void)createMobilePeer{
    if (sharedSpeakUpManager.me) {
        [sharedSpeakUpManager.me setUpdateLocationMode:OFF_MODE];//invalidate setUpdateLocation if needed
    }
    sharedSpeakUpManager.me = [[MobilePeer alloc]  initWithServerURI:@"http://isi01.unil.ch:8080/iWall-Middleware-TEST-Bridging/MobilePeer" andCallbackURI: @"http://isi01.unil.ch:8080/iWall-Middleware-TEST-Bridging/MatchHandler"];
}

// RESET PROCEDURES (called asynchronously)
// called when rooms are pulled down, or when a room has been deleted
- (void)resetData{
    NSLog(@"resetData");
    [self resetProcedureStepOne];
}
...
-(void)resetProcedureStepThree:(Response*)response{
...
-(void)subscribeToNearbyRooms{
    //subscribe to all rooms currently around (this is a one shot subscription, it could also be longer)
    NSString* roomSelector = @"eventType LIKE 'room'";
    [me subscribe:roomSelector onTopic:sharedTopic inRange:[subscriptionRadius doubleValue] isOutward:NO  forDuration:0 isMobile:NO delegate:self];
}
...
- (void)rateMessage:(NSString*)messageID inRoom:(NSString*)roomID  yesRating:(int) yesRating noRating:(int) noRating{
    ...
    [me publish:event onTopic:sharedTopic inRange:[publicationRadius doubleValue] isOutward:NO forDuration:720 isMobile:NO  delegate:self];
    [self startNetworking];
}
...
-(void) createMessage:(Message *) message{
    Event* event = [[Event alloc] init];
    [event setStringProperty:@"eventType" to:@"message"];
    [event setStringProperty:@"messageID" to:message.messageID];
    [event setStringProperty:@"specificEventType" to:@"newMessageEvent"];
    [event setStringProperty:@"roomID" to:message.roomID];
    [event setStringProperty:@"content" to:message.content];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc]init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    [event setStringProperty:@"creationTime" to:[dateFormatter stringFromDate:[NSDate date]]];
    Request* publicationRequest = [me publish:event onTopic:sharedTopic inRange:[publicationRadius doubleValue] isOutward:NO  forDuration:720 isMobile:NO delegate:self];
    message.publicationRequest=publicationRequest;
...
- (void)createRoom:(Room *)room{
    //publish a new room (not sure if we need to keep the startnetworking)
    Event *event = [[Event alloc] init];
    [event setStringProperty:@"eventType" to:@"room"];
    [event setStringProperty:@"roomID" to:room.roomID];
    [event setStringProperty:@"roomName" to:room.name];
    [event setDoubleProperty:@"roomLatitude" to:room.latitude];
    [event setDoubleProperty:@"roomLongitude" to:room.longitude];
    [event setBooleanProperty:@"isOfficial" to:room.isOfficial];
    // create a publication that represents a rating of a message
    Request* publicationRequest = [me publish:event onTopic:sharedTopic inRange:[publicationRadius doubleValue] isOutward:NO forDuration:43200 isMobile:NO  delegate:self]; // rooms last for 1 month
    room.publicationRequest=publicationRequest;
    [myRoomIDs addObject:room.roomID];
    [self savePeerData];
    [self resetProcedureStepTwo]; // the connection is ok, so do the rest, unsubscribe and subscribe again
    [self startNetworking];
}
...
-(NSArray*) deleteRoom:(Room *) room{
    ...
    if(publicationID){
        ...
        [me publish:event onTopic:sharedTopic inRange:[publicationRadius doubleValue] isOutward:NO forDuration:43200 isMobile:NO  delegate:self];
...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation{
    ...
    if(!locationIsOK && newLocation.coordinate.latitude!=-1 && newLocation.coordinate.longitude!=-1){
        locationIsOK=YES;
        [self resetProcedureStepOne];
    }
}
...
-(void)initPeerData{
    ...
    peerID = [NSNumber numberWithInt:[defaults integerForKey:@"peerID"]];
...
-(void)savePeerData{
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [defaults setInteger:[peerID intValue] forKey:@"peerID"];
